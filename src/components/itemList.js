...
const ItemsList = (props) => {
  const { data, cartItems, cart, setShowButton, getCartItems, session } = props;
  ...
  const renderItemHeader = (info) => {
    ...
    return <Image style={styles.itemHeader} source={{ uri: image_source }} />;
  };

  const renderProductItem = (info) => (
    <View style={[styles.itemCard, styles.productItem]}>
      <View style={styles.itemCardHeader}>{renderItemHeader(info)}</View>
      <View style={styles.itemCardBody}>
        <View style={{ minHeight: 50 }}>
          <Text style={styles.itemNameText}>{info.item.name}</Text>
        </View>
        <View>
